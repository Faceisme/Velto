...
Velto 是一个面向 Apple Silicon Mac 的桌面操作增强工具。它把鼠标手势、鼠标滚轮手感、窗口管理、窗口切换和输入法自动切换集中到一个常驻菜单栏 App 里，目标是减少高频操作里的重复按键和窗口切换成本。

当前项目使用 Swift Package Manager 构建，最低支持 macOS 26。App 默认以辅助应用运行，不显示 Dock 图标，通过菜单栏图标进入设置和重启监听。
